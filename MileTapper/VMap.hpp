...
namespace MileTapper {
	class VMap {
		...
		virtual void loadFromJson(const rapidjson::Document& root) = 0;
		...
		static VMap* createMapFromFile(const char* filename);
		static VMap* createMapFromStream(std::istream& in);
		static VMap* createMapFromJson(rapidjson::Document& root);
