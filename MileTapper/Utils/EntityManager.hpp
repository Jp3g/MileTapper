...
namespace MileTapper {
		...
		private:
			...
			std::vector<VPool*> _componentsEntities;
		...
		template<class ENTITY>
		template<typename ... COMPONENT>
		bool EntityManager<ENTITY>::View<COMPONENT ...>::iterator::operator==(const typename EntityManager<ENTITY>::View<COMPONENT...>::iterator& other)
		{
			return _it == other._it && _view._mask == other._view._mask && &(_view._manager) == &(other._view._manager);
		}
